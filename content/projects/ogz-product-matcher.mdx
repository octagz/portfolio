...
technologies: "Python, NLP Techniques, TF-IDF, GPT-4"
...
Nuqlea has shown promising results in automating and assisting product linking in the construction industry, significantly reducing manual efforts.

&nbsp;

# Evolution: LLM-powered Matching

After the initial TF-IDF approach proved the concept, we evolved the system by integrating GPT-4 to handle edge cases the NLP model couldn't resolve. The LLM-powered approach brought several advantages:

- **Higher accuracy on ambiguous queries**: GPT-4's contextual understanding handled product descriptions that relied on domain knowledge the statistical model lacked.
- **International scalability**: The language model could process product names and descriptions in multiple languages without separate pipelines.
- **Reduced manual work by 80%**: Combined with the original NLP pipeline, the GPT-4 integration automated the vast majority of product matching, freeing the operations team to focus on strategic tasks.

The hybrid approach — using TF-IDF for high-confidence matches and GPT-4 for ambiguous or complex cases — proved to be both cost-effective and highly accurate, enabling Nuqlea to scale its catalog operations significantly.